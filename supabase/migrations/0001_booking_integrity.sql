...
-- 1. client_request_id column (added by server code / financial.js). Add only
--    if it does not yet exist on the live database. Must be created BEFORE the
--    uniqueness constraint below references it.
do $$
begin
...
-- 2. Idempotency: a given (renter, client_request_id) can only ever produce
...
-- 3. Double-booking protection: no two confirmed/pending/active bookings may
